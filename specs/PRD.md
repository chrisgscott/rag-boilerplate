...
|---|---------|-------------|----------|
| 1 | Document Ingestion Pipeline | Upload PDFs, Markdown, DOCX, HTML, and plain text files. Parse via Docling (97.9% table accuracy, OCR). Chunk recursively at 400-512 tokens with 15% overlap. Embed via text-embedding-3-small. Store in pgvector with HNSW index. Reliable async processing via pgmq with retries and DLQ. | Must Have |
...
### Document Ingestion
- [x] File upload via Supabase Storage (PDF, Markdown, DOCX, HTML, Plain text)
...
- ~~Document OCR (scanned PDF support)~~ — **Resolved:** Docling includes OCR support

## Open Questions
1. ~~**PDF Parsing:**~~ **Decided:** Docling (MIT, 97.9% table accuracy, OCR, Python). Requires a separate Python service — adopted 3-service architecture (Next.js on Vercel + Python/FastAPI on Render + Supabase Cloud). See Decision #009.
